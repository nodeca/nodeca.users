  ...
  while (i < data.length) {
    ...
    switch (this._state) {
      ...
      case FILE_START_FF:
        ...
        this.onData(new Uint8Array([ 0xFF, 0xD8 ]));
      ...
      case SEGMENT_MARKER:
        ...
        if ((0xD0 <= b && b <= 0xD9) || b === 0x01) {
          ...
          if (this._markerCode === 0xD9 /* EOI */) {
            this.onData(new Uint8Array([ 0xFF, 0xD9 ]));
            this._state = FINAL;
            this.onEnd();
          } else {
      ...
      case SEGMENT_LENGTH:
        ...
        if (this._bytesLeft <= 0) {
          if (this._comment !== null && typeof this._comment !== 'undefined' && this._markerCode !== 0xE0) {
            ...
            buf = new Uint8Array(5 + enc.length);
            buf[0] = 0xFF;
            buf[1] = 0xFE;
      ...
      case SEGMENT_PIPE_DATA:
        di = Math.min(this._bytesLeft, data.length - i);
        this.onData(data.subarray(i, i + di));
      ...
      case SEGMENT_BUFFER:
        this._app1buffer = new Uint8Array(this._bytesLeft);
        this._app1pos    = 0;
      ...
      case SEGMENT_BUFFER_DATA:
        ...
        let buf_slice = data.subarray(i, i + di);

        this._app1buffer.set(buf_slice, this._app1pos);
      ...
      case IMAGE:
        ...
        if (!this._removeImage) {
          this.onData(data.subarray(start, i));
        }
      ...
      case IMAGE_FF:
        ...
        if (b === 0x00 || (b >= 0xD0 && b < 0xD8) || b === 0xFF) {
          if (!this._removeImage) {
            this.onData(new Uint8Array([ 255, b ]));
          }
...
JpegFilter.prototype.end = function () {
  switch (this._state) {
    ...
    default:
      ...
      //
      this.push(new Uint8Array([ 0xFF, 0xD9 ]));
  }
};
