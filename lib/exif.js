...
function getExifOrientation(offset, max) {
  ...
  if (getUint32(offset + 4) === 0x45786966) {
    ...
    if (tiffStart + 8 > max) return null;

    // Missing byte alignment offset
    if (getUint16(offset + 8) !== 0) return null;
...
function parseMetadata(rawHeader) {
  ...
  // Not jpeg
  if (getUint16(0) !== 0xffd8) return null;
  ...
  while (offset < max) {
    ...
    if ((segmentMarker >= 0xffe0 && segmentMarker <= 0xffef) || segmentMarker === 0xfffe) {
      ...
      // Invalid segment size
      if (offset + segmentLength > max) return null;
  ...
  return { orientation, header: data.subarray(0, offset) };
}
