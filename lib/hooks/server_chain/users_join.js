...
module.exports = function (N) {
  ...
  N.wire.after('server_chain:**', { priority: 50 }, function join_users(env, callback) {
    var user_id_list, result, UserModel;
    ...
    user_id_list = _.compact(env.data.users);

    env.extras.puncher.start('Users join', { count: user_id_list.length });

    result = env.response.data.users = {};
