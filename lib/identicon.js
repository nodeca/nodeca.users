...
function Identicon(str, size) {
  ...
  this.padding   = -1; // small overlap to remove thin lines between anchors
...
Identicon.prototype.drawTiles = function (matrix) {
  ...
  for (x = 0; x < this.side_tiles; x++) {
    for (y = 0; y < this.side_tiles; y++) {
      if (matrix[x][y]) {
        result.push(tpl({
          x: x * this.tileSize + this.borderSize + this.padding,
          y: y * this.tileSize + this.borderSize + this.padding,
          w: this.tileSize - 2 * this.padding,
          h: this.tileSize - 2 * this.padding
        }));
      }
    }
  }
