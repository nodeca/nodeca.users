...
module.exports = function (N) {
  ...
  //
  N.wire.before('server:**', { priority: -90 }, function force_https_to_http(env, callback) {
    if (!env.request.isEncrypted) {
  ...
  // additions to this rules. Like for auth pages.
  //
  N.wire.before('server:**', { priority: -90 }, function force_http_to_https(env, callback) {
    if (env.request.isEncrypted) {
      // Already on SSL - do nothing.
      callback();
      return;
  ...
  ], { priority: -90 }, function force_https_on_auth(env, callback) {
    if (env.request.isEncrypted) {
