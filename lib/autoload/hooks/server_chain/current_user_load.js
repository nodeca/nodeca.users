...
module.exports = function (N) {
  ...
  function initGuestSession(env, callback) {
    findUsergroupId('guests', function (err, guestsId) {
      ...
      env.user_info.user_name   = '';
      env.user_info.user_hid    = 0;
      env.user_info.user_avatar = null;
  ...
  N.wire.before('server_chain:*', { priority: -50 }, function current_user_load(env, callback) {
    ...
    N.models.users.User
      ...
      .exec(function (err, user) {
        ...
        // these properties are only stored because they're needed in runtime
        env.user_info.user_name   = user.name;
        env.user_info.user_hid    = user.hid;
        env.user_info.user_avatar = user.avatar_id;

        // copy user's locale back to session
        // (it'll be copied to runtime from there)
        env.session.locale = env.user_info.locale;
  ...
  N.wire.after('server_chain:http:*', function fill_runtime_user_info(env) {
    env.runtime.user_id     = env.user_info.user_id || '000000000000000000000000';
    env.runtime.user_hid    = env.user_info.user_hid;
    env.runtime.user_name   = env.user_info.user_name;
    env.runtime.user_avatar = env.user_info.user_avatar;
    env.runtime.is_guest    = env.user_info.is_guest;
    env.runtime.is_member   = env.user_info.is_member;
    env.runtime.locale      = env.user_info.locale;
  });
};
