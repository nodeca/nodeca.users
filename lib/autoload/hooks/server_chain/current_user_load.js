...
module.exports = function (N) {
  ...
  N.wire.before('server_chain:*', { priority: -50 }, function current_user_load(env, callback) {
    userInfo(N, env.session ? env.session.user_id : null, function (err, userInfo) {
      if (err) {
        callback(err);
        return;
      }

      env.user_info = userInfo;
      callback();
    });
  });
  ...
  N.wire.before('server_chain:*', { priority: -50 }, function fill_user_info_locale(env) {
    env.user_info.locale = env.user_info.locale || env.session.locale;
