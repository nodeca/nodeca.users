...
module.exports = function (N) {
  ...
  //
  N.wire.before('server_chain:*', { priority: -60 }, function http_force(env, callback) {
    if (!env.request.isEncrypted) {
    ...
    env.extras.puncher.start('(http_force) fetch setting');
  ...
  N.wire.after('server_chain:*', { priority: 80, ensure: true }, function ssl_cookie_set(env, callback) {
    ...
    env.extras.puncher.start('(ssl_cookie_set) fetch setting');
  ...
  N.wire.after('server_chain:*', { priority: 85, ensure: true }, function session_cookie_secure(env, callback) {
    ...
    env.extras.puncher.start('(session_cookie_secure) fetch setting');
