...
var validate = validator(GOOGLE_SCHEMA, { verbose: true });
...
module.exports = function (N) {
  ...
  N.wire.before('server:users.auth.oauth.callback', function oauth_google_requests(env, callback) {
    ...
    var client = N.config.oauth.google.client;
    var secret = N.config.oauth.google.secret;
      ...
      function (err, access_token) {
        ...
        oauth2.get(get_url, access_token, function (err, data) {
          ...
          if (!validate(provider_data)) {
            callback(format(
              'OAuth internal error, Google reply did not passed validation. (%s).',
              validate.errors
            ));
            return;
          }
