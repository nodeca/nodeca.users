...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_last_active_update() {
    ...
    N.queue.registerTask({
      ...
      async process() {
        try {
          let items = await N.redis.zrangebyscoreAsync('users:last_active', '-inf', '+inf', 'withscores');
          ...
          if (items.length) {
            ...
            await bulk.execute();
            await N.redis.zremrangebyscoreAsync('users:last_active', '-inf', items[items.length - 1]);
          }
        } catch (err) {
