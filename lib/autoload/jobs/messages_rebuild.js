...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_messages_rebuild() {
    ...
    N.queue.registerTask({
      ...
      async init() {
        let query = N.models.users.DlgMessage.countDocuments();

        if (this.args.length < 1 || !this.args[0]) {
          // if no min _id
          let min_msg = await N.models.users.DlgMessage.findOne()
                                  .select('_id')
                                  .sort({ _id: 1 })
                                  .lean(true);
          ...
          this.args[0] = String(min_msg._id);
        } else {
        ...
        if (this.args.length < 2 || !this.args[1]) {
          // if no max _id
          let max_msg = await N.models.users.DlgMessage.findOne()
                                  .select('_id')
                                  .sort({ _id: -1 })
                                    .lean(true);
          ...
          this.args[1] = String(max_msg._id);
        } else {
    ...
    N.queue.registerTask({
      ...
      async process(ids) {
        ...
        await N.wire.emit('internal:users.message_rebuild', ids);
