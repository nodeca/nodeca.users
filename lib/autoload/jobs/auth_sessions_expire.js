    ...
    N.queue.registerTask({
      ...
      process: Promise.coroutine(function* () {
        ...
        try {
          ...
          let sessions = yield N.models.users.AuthSession.find()
                                   .where('first_ts').lte(Date.now() - expireDays.valueOf() * 24 * 60 * 60 * 1000)
                                   .lean(true);

          if (!sessions.length) return;

          yield archive_session(N, _.map(sessions, 'session_id'), N.models.users.AuthSessionLog.logout_types.EXPIRED);
