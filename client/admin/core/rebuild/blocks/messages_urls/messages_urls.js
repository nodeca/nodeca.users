// Display urls extraction progress in admin interface
//
'use strict';
...
var view = null;
var SELECTOR = '#rebuild-messages-urls-task';
var ignore_runid = 0;
var last_runid = 0;
...
N.wire.on('navigate.done:admin.core.rebuild', function rebuild_messages_urls_widget_setup() {
  if (!$(SELECTOR).length) return;

  var current = N.runtime.page_data.messages_urls_task.current || 0;
  var total   = N.runtime.page_data.messages_urls_task.total || 1;
  ...
  N.live.on('admin.core.rebuild.messages_urls', update_task_status);
});


N.wire.on('navigate.exit:admin.core.rebuild', function rebuild_messages_urls_widget_teardown() {
  if (!$(SELECTOR).length) return;

  view = null;
  ko.cleanNode($(SELECTOR)[0]);

  N.live.off('admin.core.rebuild.messages_urls', update_task_status);
});


N.wire.once('navigate.done:admin.core.rebuild', function rebuild_messages_urls_widget_setup_handlers() {
  ...
  N.wire.on(module.apiPath + '.start', function rebuild_start() {
    ...
    N.io.rpc('admin.core.rebuild.messages_urls.start')
      .then(function () {
  ...
  N.wire.on(module.apiPath + '.stop', function rebuild_stop() {
    ...
    N.io.rpc('admin.core.rebuild.messages_urls.stop')
      .then(function () {
        // reset progress bar to zero,
        // and ignore all updates on the last task
        ignore_runid = Math.max(prev_runid, ignore_runid);
