...
module.exports = function (N, apiPath) {
  ...
  N.wire.on(apiPath, async function rebuild_message(id) {
    let post = await N.models.users.DlgMessage.findById(id);
    ...
    await N.models.users.DlgMessage.update({ _id: post._id }, updateData);

    //
    // If it's the last message in the dialog, rebuild preview as well
    //
    let dialog = await N.models.users.Dialog.findById(post.parent);
    ...
    if (dialog && dialog.cache && String(dialog.cache.last_message) === String(post._id)) {
      ...
      await N.models.users.Dialog.update({ _id: post.parent }, {
        $set: {
          cache: {
